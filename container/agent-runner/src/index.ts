...
async function main(): Promise<void> {
  ...
  // Save images to files for debugging and reference in prompt
  if (containerInput.images && containerInput.images.length > 0) {
    const imageDir = '/workspace/group/_images';
    fs.mkdirSync(imageDir, { recursive: true });
    for (let i = 0; i < containerInput.images.length; i++) {
      const img = containerInput.images[i];
      const ext = img.mediaType === 'image/png' ? 'png' : 'jpg';
      const filepath = path.join(imageDir, `image_${Date.now()}_${i}.${ext}`);
  ...
  if (containerInput.documents && containerInput.documents.length > 0) {
    const docsDir = '/workspace/group/_documents';
    fs.mkdirSync(docsDir, { recursive: true });
    const docPaths: string[] = [];
    for (let i = 0; i < containerInput.documents.length; i++) {
      const doc = containerInput.documents[i];
    ...
    prompt += `\n\n[Note: This message includes ${containerInput.documents.length} document(s). Use the Read tool to view: ${docRefs}]`;
  }
